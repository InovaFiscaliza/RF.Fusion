...
SECRET_FILE = "/root/RF.Fusion/test/appCataloga/.secret"
...
def ssh_connect(conn_data: dict) -> paramiko.SSHClient:
    ...
    paramiko.util.log_to_file(filename="/root/RF.Fusion/test/appCataloga/paramiko.log", level=logging.DEBUG)

    print(  f"\n- Connecting to:\n"
            f"     Hostmanme: {conn_data['ssh_proxy']}\n"
            f"     Port:      {conn_data['port']}\n"
            f"     User:      {conn_data['user_full_name']}\n"
            f"     Pass:      {conn_data['password']}\n")

    try:
        client.connect(hostname=conn_data['ssh_proxy'],
                       port=conn_data['port'],
                       username=conn_data['user_full_name'],
...
def load_config(client):
    ...
    # ! STUCK HERE ! Senhasegura MFA is blocking the execution of the command above. Submitting MFA along with user information is not working as supposed.
    # Try interactive mode for logging in
