...
def process_due_backup(
    ...
    if due_backup_str:
        ...
        # Create file tasks into HOST_TASK table
        # File task can be 1) FILE_TASK_BACKUP_TYPE or FILE_FASK_PROCESSING_TYPE
        if task["task_nu_type"] == db_bp.FILE_TASK_BACKUP_TYPE:
            db_bp.file_task_create(
                host_id=task["host_id"],
                volume=task["host_uid"],
                task_status= db_bp.TASK_PENDING,
                task_type=db_bp.FILE_TASK_BACKUP_TYPE,
                host_backup=True,
                files=due_backup_list,
            )
        # Create a metadata task into METADATA_TASK table    
        elif task["task_nu_type"] == db_bp.FILE_TASK_METADATA_TYPE:
            db_bp.file_task_create(
                host_id=task["host_id"],
                volume=task["host_uid"],
                task_status= db_bp.TASK_PENDING,
                task_type=db_bp.FILE_TASK_METADATA_TYPE,
                host_backup=task["host_backup"],
                files=due_backup_list,
            )
