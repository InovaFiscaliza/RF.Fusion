...
def move_file_set(files_to_move: set, target: str, log: sh.log) -> None:
    ...
    if len(files_to_move) > 1:
        user_input = input(
            "Do you wish to confirm each entry before move operation? (y/n): "
        )
...
def refresh_repo_files(log: sh.log) -> None:
    ...
    if len(files_not_in_repo) > 0:
        log.entry(
            f"{len(files_not_in_repo)} files not in the repository but in the database"
        )
        confirmation = input(
            "Do you want to remove database entries that are missing the correspondent file? (y/n): "
        )
        ...
        if confirmation.lower() == "y":
            ...
            log.entry(
                "No entry in the RFDATA database without correspondent file in the repository."
            )

    if len(files_not_in_rfdb) > 0:
        log.entry(
            f"{len(files_not_in_rfdb)} files not in the RFDATA database but in the repository"
        )
        confirmation = input(
            "Do you want to move files to TMP_FOLDER for later reprocessing? (y/n): "
        )
        ...
        log.entry(
            "No file in the repository without correspondent entry in the RFDATA database."
        )
...
def refresh_tmp_files(log: sh.log) -> None:
    ...
    db_tmp_files = db_bp.list_bpdb_files(
        task_status=db_bp.TASK_PENDING, task_type=db_bp.FILE_TASK_PROCESS_TYPE
    )

    log.entry(f"{len(repo_tmp_files)} files in the repository TMP_FOLDER:")
    log.entry(
        f"{len(db_tmp_files)} database entries related to repository TMP_FOLDER files."
    )
    ...
    if len(files_missing_in_tmp) > 0:
        log.entry(
            f"{len(files_missing_in_tmp)} files missing in the TMP_FOLDER but in the database"
        )
        confirmation = input(
            "Do you want to remove database entries that are missing the correspondent file? (y/n): "
        )
        ...
        log.entry(
            "No entry in the BPDATA database without correspondent file in the TMP_FOLDER."
        )
    ...
    if len(files_to_be_processed) > 0:
        log.entry(
            f"{len(files_to_be_processed)} files in the TMP_FOLDER but not in the task list to be processed"
        )
        confirmation = input(
            f"Do you want to add these {len(files_to_be_processed)} files into the processing queue? (y/n): "
        )
...
def refresh_trash_files(log: sh.log) -> None:
    ...
    db_trash_files = db_bp.list_bpdb_files(
        task_status=db_bp.TASK_ERROR, task_type=db_bp.FILE_TASK_PROCESS_TYPE
    )
    ...
    log.entry(
        f"{len(db_trash_files)} database entry(ies) related to repository TRASH_FOLDER files."
    )

    if len(files_missing_in_trash) > 0:
        log.entry(
            f"{len(files_missing_in_trash)} file(s) missing in the TRASH_FOLDER but in the database"
        )
        confirmation = input(
            "Do you want to remove database entries that are missing the correspondent file? (y/n): "
        )
        ...
        log.entry(
            "No entry in the BPDATA database without correspondent file in the TRASH_FOLDER."
        )
    ...
    if len(files_spilled_from_trash) > 0:
        ...
        log.entry(
            f"{len(files_spilled_from_trash)} file(s) found in TRASH_FOLDER but not in the database"
        )
        ...
        while not finish_cleaning:
            global_option = input(
                "Do you want to re(P)rocess all, (D)elete all or (C)onfirm each entry? (p/d/c): "
            )
            match global_option.lower():
                case "p":
                    confirmation = input(
                        f"This will reprocess {len(files_spilled_from_trash)} file(s) from TRASH_FOLDER, moving then to TMP_FOLDER. Are you sure? (y/n): "
                    )
                    if confirmation.lower() == "y":
                        handle_trash.move()
                ...
                case "d":
                    confirmation = input(
                        f"This delete {len(files_spilled_from_trash)} file(s) from TRASH_FOLDER. Are you sure? (y/n): "
                    )
                    if confirmation.lower() == "y":
                        handle_trash.delete()
                        finish_cleaning = True
                ...
                case "c":
                    for path, filename in files_spilled_from_trash:
                        ...
                        ask_again = True
                        task_summary = {"move": 0, "delete": 0, "skip": 0}
                        while ask_again:
                            single_option = input(
                                f"re(P)rocess, (D)elete or (S)kip file: '{path}/{filename}'? (p/d/s): "
                            )
                            ...
                            match single_option.lower():
                                ...
                                case _:
                                    log.entry(
                                        f"Invalid option '{single_option}'. Try again."
                                    )

                    log.entry(
                        f"{task_summary['move']} file(s) moved, {task_summary['delete']} deleted, {task_summary['skip']} skipped."
                    )

                    confirmation = input(
                        "You may review again the skipped file(s). [F]inish trash processing or go [A]gain through the skipped files? (F/A): "
                    )
...
def refresh_total_files(log: sh.log) -> None:
    ...
    for host_id, equip_id, host_uid in host_id_list:
        if equip_id is None:
            log.entry(
                f"Host '{host_uid}' has no equipment associated. Retrieving RFDB entry is not possible"
            )
            continue

        rfm_file_count = db_rfm.count_rfm_host_files(
            equipment_id=equip_id, volume=k.REPO_UID
        )

        (pending_processing, error_processing) = db_bp.count_bp_host_files(
            host_id=host_id
        )
