...
LOAD DATA INFILE '/srv/app_volume/tmp/appCataloga/equipmentType.csv'
...
LOAD DATA INFILE '/srv/app_volume/tmp/appCataloga/IBGE-BR_UF_2020_BULKLOAD.csv'
...
LOAD DATA INFILE '/srv/app_volume/tmp/appCataloga/IBGE-BR_Municipios_2020_BULKLOAD.csv'
...
CREATE TABLE DIM_SPECTRUM_SITE (
    ...
    GEO_POINT POINT NOT NULL COMMENT 'Location tuple (longitude, latitude)',
...
LOAD DATA INFILE '/srv/app_volume/tmp/appCataloga/fileType.csv'
...
CREATE TABLE DIM_SPECTRUM_FILE (
    ...
    -- New file information got from SSH
...
CREATE TABLE DIM_SPECTRUM_DETECTOR (
    ID_DETECTOR INT NOT NULL AUTO_INCREMENT COMMENT 'Primary Key to measurement detector type dimension table', 
    NA_DETECTOR VARCHAR(50) COMMENT 'Description of the measurement detector type. e.g. RMS, Sample, Positive Peak',
...
LOAD DATA INFILE '/srv/app_volume/tmp/appCataloga/measurementUnit.csv'
...
CREATE TABLE FACT_SPECTRUM (
    ID_SPECTRUM INT NOT NULL AUTO_INCREMENT COMMENT 'Primary Key to the fact table of spectrum measurement data.',
    FK_SITE INT COMMENT 'Foreign key to location dimension table',
    FK_DETECTOR INT COMMENT 'Foreign key to location dimension table',
    ...
    CONSTRAINT FK_SITE_CONSTRAINT FOREIGN KEY (FK_SITE) REFERENCES DIM_SPECTRUM_SITE (ID_SITE),
    CONSTRAINT FK_DETECTOR_CONSTRAINT FOREIGN KEY (FK_DETECTOR) REFERENCES DIM_SPECTRUM_DETECTOR (ID_DETECTOR),
    CONSTRAINT FK_TRACE_TYPE_CONSTRAINT FOREIGN KEY (FK_TRACE_TYPE) REFERENCES DIM_SPECTRUM_TRACE_TYPE (ID_TRACE_TYPE),
    CONSTRAINT FK_MEASURE_UNIT_CONSTRAINT FOREIGN KEY (FK_MEASURE_UNIT) REFERENCES DIM_SPECTRUM_UNIT (ID_MEASURE_UNIT),
    CONSTRAINT FK_PROCEDURE_CONSTRAINT FOREIGN KEY (FK_PROCEDURE) REFERENCES DIM_SPECTRUM_PROCEDURE (ID_PROCEDURE),
    PRIMARY KEY (ID_SPECTRUM)
);

-- BRIDGE TABLES TO ALLOW FOR N-N RELATIONSHIPS TO THE FACT TABLE
CREATE TABLE BRIDGE_SPECTRUM_EQUIPMENT (
    FK_EQUIPMENT INT COMMENT 'Foreign key to equipment table', 
    FK_SPECTRUM INT COMMENT 'Foreign key to spectrum measurement table',
    CONSTRAINT FK_EQUIPMENT_SPEC_CONSTRAINT FOREIGN KEY (FK_EQUIPMENT) REFERENCES DIM_SPECTRUM_EQUIPMENT (ID_EQUIPMENT),
    CONSTRAINT FK_SPECTRUM_EQUI_CONSTRAINT FOREIGN KEY (FK_SPECTRUM) REFERENCES FACT_SPECTRUM (ID_SPECTRUM),
    PRIMARY KEY (FK_EQUIPMENT, FK_SPECTRUM)
);
...
CREATE TABLE BRIDGE_SPECTRUM_EMITTER (
    ...
    FK_SPECTRUM INT COMMENT 'Foreign key to spectrum measurement table', 
    CONSTRAINT FK_EMITTER_SPEC_CONSTRAINT FOREIGN KEY (FK_EMITTER) REFERENCES DIM_SPECTRUM_EMITTER (ID_EMITTER), 
    CONSTRAINT FK_SPECTRUM_EMIT_CONSTRAINT FOREIGN KEY (FK_SPECTRUM) REFERENCES FACT_SPECTRUM (ID_SPECTRUM), 
...
CREATE TABLE BRIDGE_SPECTRUM_FILE (
    ...
    FK_SPECTRUM INT COMMENT 'Foreign key to spectrum measurement table', 
    CONSTRAINT FK_FILE_SPEC_CONSTRAINT FOREIGN KEY (FK_FILE) REFERENCES DIM_SPECTRUM_FILE (ID_FILE), 
    CONSTRAINT FK_SPECTRUM_FILE_CONSTRAINT FOREIGN KEY (FK_SPECTRUM) REFERENCES FACT_SPECTRUM (ID_SPECTRUM), 
