...
-- ==========================================================
-- METADATA_TASK (DISCOVERY STAGING)
-- ==========================================================
CREATE TABLE METADATA_TASK (
    ID_METADATA_TASK INT NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT 'Primary Key to metadata task table',
    FK_HOST INT NOT NULL COMMENT 'Foreign key to host table',
    DT_METADATA_TASK DATETIME NOT NULL COMMENT 'Date/time of discovery',
    ...
    NA_EXTENSION VARCHAR(20) COMMENT 'File extension',
    ...
    FL_BACKUP_REQUESTED TINYINT DEFAULT 0 COMMENT '0=Not requested, 1=Also trigger backup after discovery',
	NU_STATUS TINYINT DEFAULT 0 COMMENT '-1=Error; 0=Pending; 1=In Progress; 2=Completed',
    NU_PID INT COMMENT 'Process ID if running',
    NA_MESSAGE TEXT COMMENT 'Error message or info',
    CONSTRAINT FK_METADATA_TASK_HOST FOREIGN KEY (FK_HOST) REFERENCES HOST(ID_HOST)
);

-- ==========================================================
-- FILE_TASK (BACKUP QUEUE)
-- ==========================================================
CREATE TABLE FILE_TASK (
    ID_FILE_TASK INT NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT 'Primary Key to file task table',
    FK_HOST INT NOT NULL COMMENT 'Foreign key to host table',
    DT_FILE_TASK DATETIME NOT NULL COMMENT 'Date/time when task was created',
    NA_HOST_FILE_PATH VARCHAR(3000) COMMENT 'Path to the file in the host',
    NA_HOST_FILE_NAME VARCHAR(100) COMMENT 'Name of the file in the host',
    NU_HOST_FILE_MD5 VARCHAR(32) COMMENT 'MD5 hash of the file in the host',
...
CREATE TABLE FILE_TASK_HISTORY (
    ...
    FK_FILE_TASK INT COMMENT 'Reference to FILE_TASK if needed',
    FK_HOST INT NOT NULL COMMENT 'Foreign key to host table',
    DT_BACKUP DATETIME NOT NULL COMMENT 'Date/time when backup was completed',
    ...
    NU_STATUS TINYINT COMMENT '-1=Error; 2=Completed',
