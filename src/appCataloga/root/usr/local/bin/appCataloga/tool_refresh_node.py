...
def refresh_tmp_files(log: sh.log) -> None:
    ...
    db_tmp_files = db_bp.list_bpdb_files(
        task_status=db_bp.TASK_PENDING, task_type=db_bp.FILE_TASK_BACKUP_TYPE
    )
...
def refresh_trash_files(log: sh.log) -> None:
    ...
    db_trash_files = db_bp.list_bpdb_files(
        task_status=db_bp.TASK_ERROR, task_type=db_bp.FILE_TASK_BACKUP_TYPE
    )
    ...
    log.entry(
        f"{len(db_trash_files)} database entries related to repository TRASH_FOLDER files.\n"
    )

    if len(files_missing_in_trash) > 0:
        log.entry(
            f"{len(files_missing_in_trash)} files missing in the TRASH_FOLDER but in the database"
        )
        confirmation = input(
            "Do you want to remove database entries that are missing the correspondent file? (y/n): "
        )
    ...
    if len(files_spilled_from_trash) > 0:
        ...
        log.entry(
            f"{len(files_spilled_from_trash)} files in TRASH_FOLDER that are not in the database"
        )
        ...
        while not finish_cleaning:
            ...
            match global_option.lower():
                case "p":
                    confirmation = input(
                        "This will reprocess len(files_spilled_from_trash) files from TRASH_FOLDER, moving then to TMP_FOLDER. Are you sure? (y/n): "
                    )
                    if confirmation.lower() == "y":
                        handle_trash.move()
                ...
                case "d":
                    confirmation = input(
                        "This delete len(files_spilled_from_trash) files from TRASH_FOLDER. Are you sure? (y/n): "
                    )
                    if confirmation.lower() == "y":
                        handle_trash.delete()
                        finish_cleaning = True
                ...
                case "c":
                    confirmation = input(
                        "Do you want to confirm operation for each file? (y/n): "
                    )
                    if confirmation.lower() == "y":
                        for filepath, filename in files_spilled_from_trash:
                            ...
                            while ask_again:
                                single_option = input(
                                    f"re(P)rocess {filepath}/{filename}, (D)elete or (S)kip it? (p/d/s): "
                                )
