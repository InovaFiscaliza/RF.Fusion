...
CREATE TABLE HOST (
    ...
    NO_HOST_ADD VARCHAR(50),
    NO_HOST_USER VARCHAR(50),
    NO_HOST_PASSWORD VARCHAR(50)
    NU_HOST_FILES BIGINT
    NU_PENDING_BACKUP INT
    DT_LAST_BACKUP DATETIME
);

CREATE TABLE FILE_LIST (
    ID_FILE_LIST INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
    ID_HOST INT,
    NO_FILE_PATH VARCHAR(3000),
    NO_FILE_NAME VARCHAR(100),
    DT_FILE_CREATION DATETIME,
    DT_FILE_LAST_MODIFICATION DATETIME,
    DT_FILE_LAST_ACCESS DATETIME,
    DT_FILE_LAST_BACKUP DATETIME,
    NU_FILE_BACKUP_STATUS INT,
    CONSTRAINT FK_FILE_LIST_HOST FOREIGN KEY (ID_HOST) REFERENCES HOST (ID_HOST)
);
