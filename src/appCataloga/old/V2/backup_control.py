    ...
    while True:
        ...
        if task:
            ...
            if new_task and (task_counter < k.BKP_MAX_PROCESS):
                ...
                task["process_handle"] = subprocess.Popen(
                    [command],
        ...
        if len(tasks) > 0:
            ...
            for running_task in tasks:
                ...
                try:
                    task_output, task_error = running_task[
                        "process_handle"
                    ...
                    if (
                        time.time() - running_task["birth_time"]
                        > k.HOST_TASK_EXECUTION_TIMEOUT
                    ):
                        running_task["process_handle"].kill()
                        tasks.remove(running_task)
                        log.warning(
                            f"Backup task canceled due to timeout {task['host_add']}"
                        )
                    pass
                ...
                if task_output:
                    try:
                        ...
                        db.host_update(
                            ...
                            pending_processing=len(
                                task_dict_output["done_backup_list"]
                            ),
                        )
                        ...
                        log.error(
                            f"Malformed JSON received. Dumped\n***Output: {task_output}\n***Error: {e}"
                        )
                    ...
                    log.entry(
                        f"Error in backup from {tasks['host_add']}. Will try again later. Error: {task_error}"
                    )
                ...
                if execution_time > k.HOST_TASK_EXECUTION_TIMEOUT:
                    ...
                    db.host_task_delete(running_task)
                    ...
                    log.warning(
                        f"Backup task killed due to timeout for host {task['host_add']} after {execution_time/60} minutes."
                    )

            log.entry(
                f"Wainting for {len(tasks)} backup tasks to finish. Next check in {k.MAX_HOST_TASK_WAIT_TIME} seconds."
            )
            ...
            log.entry(
                f"No backup task. Waiting for {k.HOST_TASK_REQUEST_WAIT_TIME/k.SECONDS_IN_MINUTE} minutes."
            )
