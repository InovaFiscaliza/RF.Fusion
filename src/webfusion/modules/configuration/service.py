...
def get_catalog() -> dict[str, list[dict[str, Any]]]:
    ...
    catalog = build_client().list_catalog()
...
def get_appcataloga_problems() -> list[dict[str, Any]]:
    """Load current appCataloga problems from the configured Zabbix API."""
    return build_client().list_appcataloga_problems()


def get_zabbix_api_url() -> str:
    """Return the configured Zabbix API endpoint for read-only integrations."""
    return get_api_url()
...
def get_hosts_backup_defaults(
    ...
    configurations = build_client().get_host_configurations(selected_host_ids)
    return {
